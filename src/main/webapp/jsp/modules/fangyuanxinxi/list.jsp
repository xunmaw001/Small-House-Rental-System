...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">房源信息管理</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-two"><span>房源信息列表</span></li>
			</ol>
		</div>
		<!-- /Breadcrumb -->
		...
		<div id="center-container">
			...
			<div class="center-form">
				<div class="form-item">
					<label>是否装修</label>
					<select name="shifouzhuangxiu" id="shifouzhuangxiuSelect" class="form-control form-control-sm" aria-controls="tableId">
						<option value=" "></option>
				...
				<div class="form-item">
					<label>月租/元</label>
					<div class="item-body">
						<input type="text" id="yuezu_start" class="begin form-control form-control-sm" placeholder="最小月租/元" aria-controls="tableId">
						<span>至</span>
						<input type="text" id="yuezu_end" class="end form-control form-control-sm" placeholder="最大月租/元" aria-controls="tableId">
					</div>
				</div>
				<div class="form-item">
					<label>房源类型</label>
					<select name="fangyuanleixing" id="fangyuanleixingSelect" class="form-control form-control-sm" aria-controls="tableId">
					</select>
				</div>
				<div class="form-item">
					<label>联系电话</label>
					<input type="text" id="lianxidianhuaSearch" class="form-control form-control-sm" placeholder="请输入联系电话" aria-controls="tableId">
				</div>
				<div class="form-item">
					<label>审核状态</label>
			...
			<table id="tableId" class="table table-bordered">
				<thead>
					<tr>
						...
						<th onclick="sort('shifouzhuangxiu')">是否装修<i id="shifouzhuangxiuIcon" class="fa fa-sort"></i></th>
						...
						<th onclick="sort('chaoxiang')">朝向<i id="chaoxiangIcon" class="fa fa-sort"></i></th>
						<th>封面</th>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			var shifouzhuangxiuIndex = document.getElementById("shifouzhuangxiuSelect").selectedIndex;
			var shifouzhuangxiuOptions = document.getElementById("shifouzhuangxiuSelect").options;
			var shifouzhuangxiuSelectedOption = shifouzhuangxiuOptions[shifouzhuangxiuIndex]
			if(shifouzhuangxiuSelectedOption.text != null && shifouzhuangxiuSelectedOption.text != ''){
				searchForm.shifouzhuangxiu = shifouzhuangxiuSelectedOption.text;
			}
			if($('#yuezu_start').val() != null && $('#yuezu_start').val() != ''){
				searchForm.yuezu_start = $('#yuezu_start').val();
			}
			if($('#yuezu_end').val() != null && $('#yuezu_end').val() != ''){
				searchForm.yuezu_end = $('#yuezu_end').val();
			}
			var fangyuanleixingIndex = document.getElementById("fangyuanleixingSelect").selectedIndex;
			var fangyuanleixingOptions = document.getElementById("fangyuanleixingSelect").options;
			var fangyuanleixingSelectedOption = fangyuanleixingOptions[fangyuanleixingIndex]
			if(fangyuanleixingSelectedOption.text != null && fangyuanleixingSelectedOption.text != ''){
				searchForm.fangyuanleixing = fangyuanleixingSelectedOption.text;
			}
        ...
        function getDataList() {
			http("fangyuanxinxi/page","GET",{
				...
				sort: sortColumn,
				order: sortOrder,
				shifouzhuangxiu : searchForm.shifouzhuangxiu,	
				yuezu_start : searchForm.yuezu_start,
				yuezu_end : searchForm.yuezu_end,
		...
		function setDataRow(item,number){
			...
			var shifouzhuangxiuCell  = document.createElement('td');

						 shifouzhuangxiuCell.innerHTML = item.shifouzhuangxiu;

			row.appendChild(shifouzhuangxiuCell);
			...
			var chaoxiangCell  = document.createElement('td');

						 chaoxiangCell.innerHTML = item.chaoxiang;

			row.appendChild(chaoxiangCell);
			...
			var peitaoxiangqingCell  = document.createElement('td');
			...
			var crossBtn0 = document.createElement('button');
            		crossBtn0.innerHTML = "预定"
			crossBtn0.setAttribute("type","button");
			crossBtn0.setAttribute("class","btn btn-primary btn-sm");
			crossBtn0.setAttribute("onclick","crossTable(" + JSON.stringify(item) + ",'yudingfangzi','','','')");
		...
		function sh(){
			...
			httpJson("fangyuanxinxi/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("fangyuanxinxi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function discussList(id) {
            ...
            window.location.href = "../discussfangyuanxinxi/list.jsp"
        }
		//清除会重复渲染的节点
        function clear(){
	...
	function fangyuanleixingSelect(){
		...
		http("option/fangyuanleixing/fangyuanleixing","GET",{},(res)=>{
			if(res.code == 0){
				...
				fangyuanleixingSelect.appendChild(nullOption);
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"fangyuanxinxi");
