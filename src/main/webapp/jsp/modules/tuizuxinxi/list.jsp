...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">退租信息管理</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-two"><span>退租信息列表</span></li>
			</ol>
		</div>
		<!-- /Breadcrumb -->
			...
			<table id="tableId" class="table table-bordered">
				<thead>
					<tr>
						...
						<th onclick="sort('yajin')">押金<i id="yajinIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('tuizushijian')">退租时间<i id="tuizushijianIcon" class="fa fa-sort"></i></th>
						<th>情况说明</th>
	...
	<script>
		...
		function setDataRow(item,number){
			...
			var tuizushijianCell  = document.createElement('td');

						  tuizushijianCell.innerHTML = item.tuizushijian;


			row.appendChild(tuizushijianCell);
			var qingkuangshuomingCell  = document.createElement('td');

						  if(item.qingkuangshuoming != null && item.qingkuangshuoming != "" && item.qingkuangshuoming.length >= 11){
			  	qingkuangshuomingCell.innerHTML = item.qingkuangshuoming.substring(0, 10) + "..."; 
			  }else{
			  	qingkuangshuomingCell.innerHTML = item.qingkuangshuoming;
			  }


			row.appendChild(qingkuangshuomingCell);
		...
		function sh(){
			...
			httpJson("tuizuxinxi/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("tuizuxinxi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"tuizuxinxi");
